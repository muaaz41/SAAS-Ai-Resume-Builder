...
export default function Builder() {
  ...
  const S = {
    page: {
      ...
      height: "100vh",
      background: [
        "radial-gradient(ellipse 58% 50% at 0% 0%, #c5dcfb 0%, rgba(197, 220, 251, 0.55) 45%, transparent 70%)",
        "radial-gradient(ellipse 58% 50% at 100% 100%, #c5dcfb 0%, rgba(197, 220, 251, 0.55) 45%, transparent 70%)",
        "radial-gradient(ellipse 75% 65% at 100% 0%, #ffffff 0%, rgba(255, 255, 255, 0.92) 35%, transparent 68%)",
        "radial-gradient(ellipse 75% 65% at 0% 100%, #ffffff 0%, rgba(255, 255, 255, 0.92) 35%, transparent 68%)",
        "radial-gradient(ellipse 88% 78% at 50% 50%, #ffffff 0%, #f9fcff 42%, #eef4ff 100%)",
      ].join(", "),
      padding: 0,
      overflow: "hidden",
      boxSizing: "border-box",
    ...
    step: (active, done) => ({
      ...
      height: 34,
      borderRadius: 999,
      border: `3px solid ${done ? "#16a34a" : "#0b72ff"}`,
      background: done ? "#16a34a" : "#ffffff",
      color: done ? "#ffffff" : "#0b72ff",
      fontWeight: "var(--font-weight-bold)",
      display: "grid",
      placeItems: "center",
      boxShadow: active ? "0 0 0 4px rgba(11,114,255,0.12)" : "none",
      cursor: "pointer",
      fontSize: 14,
    }),
    stepLabel: (active, done) => ({
      ...
      lineHeight: "14px",
      textAlign: "center",
      color: active ? "#0b72ff" : done ? "#15803d" : "#334155",
    ...
    previewTop: {
      padding: "10px 12px",
      borderBottom: "1px solid rgba(155, 184, 224, 0.55)",
      display: "flex",
      alignItems: "center",
      gap: 8,
      background:
        "linear-gradient(180deg, rgba(255,255,255,0.92) 0%, rgba(245,250,255,0.72) 45%, rgba(230,242,255,0.55) 100%)",
      borderRadius: 10,
    ...
    card: {
      ...
      borderRadius: 10,
      background: "linear-gradient(180deg, #f5f9ff 0%, #ffffff 14%)",
      width: "100%",
      height: "100%",
      overflow: "hidden",
      marginTop: 6,
    ...
    <div style={S.page}>
      ...
      <div style={S.left}>
        {/* <div style={{ fontSize: 13, fontWeight: 600, color: "#334155", marginBottom: 8 }}>
          {`${Math.min(step, 7)} of 7 steps completed`}
        </div> */}
        {/* Stepper */}
        <div style={S.stepperWrap}>
          {[1, 2, 3, 4, 5, 6, 7].map((i, idx) => (
            <React.Fragment key={i}>
              <div style={S.stepItem}>
                ...
                <span style={S.stepLabel(i === step, i < step)}>{stepTitles[i - 1]}</span>
              </div>
              {idx < 6 && <div style={S.stepLine(i < step)}>...</div>}
            </React.Fragment>
          ))}
        </div>
        <div
      ...
      >
        ...
        <div style={{ fontSize: 13, color: "#64748b", marginTop: 8 }}>
          ...
          {isInitializing || (resumeId && !serverPreviewUrl && !serverPreview) || (!resumeId && !token && !serverPreview && resume.templateSlug) ? (
            ...
            <iframe title="preview" srcDoc={injectTemplatePreviewHtml(serverPreview)} style={S.iframe} />
          ) : (
            <iframe title="preview" srcDoc={injectTemplatePreviewHtml(previewHtml)} style={S.iframe} />
          )}
        </div>
        <style>{`
          @keyframes spin {
