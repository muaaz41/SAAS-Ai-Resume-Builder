...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Always fetch templates (public endpoint)
        const templatePromise = api.get(
        ...
        const resumePromise = token 
          ? api.get("/api/v1/resumes").catch(err => {
              // If request fails (e.g., no auth), return empty data
        ...
        // Show warning if approaching resume limit (only once per session and if authenticated)
        const resumeCount = r.data?.data?.count || resumeItems.length;
        if (token && resumeCount >= 4 && !toastShownRef.current) {
  ...
  const handleSelectTemplate = async (template) => {
    ...
    // If user is not authenticated, navigate directly to builder (will use localStorage)
    if (!token) {
      navigate("/builder", {
        state: {
          startFresh: true,
    ...
    // Check resume limit before creating (only for authenticated users)
    if (resumes.length >= 5) {
    ...
    try {
      ...
      // All templates are now accessible - just show error message
          ...
          {loading ? (
            ...
            <div
              ...
              {filtered.map((t) => {
                ...
                // Remove restrictions - all templates are now accessible
                const locked = false;

                const handleSelect = resumes.length >= 5 && token
                  ? () => {
                      showToast("You have reached the maximum limit of 5 resumes. Please delete a resume to create a new one.", {
                        type: "error",
                        duration: 5000,
